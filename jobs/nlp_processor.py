...
======================
Module xử lý NLP cho việc matching Job.

Chức năng:
1. Text Cleaning - Tiền xử lý văn bản
2. Language Detection - Phát hiện ngôn ngữ (Việt/Anh)
3. Tokenization - Tách từ
   - Underthesea: Tiếng Việt
   - TextBlob: Tiếng Anh
4. TF-IDF Vectorization - Vector hóa văn bản
5. Cosine Similarity - Tính độ tương đồng
"""
...
# Args: text: Văn bản cần phát hiện ngôn ngữ
# Returns: 'vi' cho tiếng Việt, 'en' cho tiếng Anh
...
def tokenize_text(text: str, language: Optional[str] = None) -> List[str]:
    """
    Tách từ văn bản dựa trên ngôn ngữ.
    
    - Tiếng Việt: Sử dụng Underthesea
    - Tiếng Anh: Sử dụng TextBlob
    
    Args:
        text: Văn bản cần tách từ
        language: Ngôn ngữ ('vi' hoặc 'en'). Nếu None sẽ tự phát hiện.
...
def get_tokenized_text(text: str, language: Optional[str] = None) -> str:
    """
    Tách từ và trả về text đã tokenize (space-separated).
...
def calculate_tfidf_similarity(text1: str, text2: str, use_tokenization: bool = True) -> float:
    ...
    1. Detect language
    2. Clean text
    3. Tokenize (Underthesea for Vietnamese, TextBlob for English)
    ...
    Args:
        ...
        use_tokenization: Có sử dụng tokenization hay không
    ...
    try:
        # Combine texts to detect dominant language
        ...
        if use_tokenization:
            ...
            # Just clean without tokenization
...
def get_top_keywords(text: str, top_n: int = 20, language: Optional[str] = None) -> List[Tuple[str, float]]:
    ...
    Args:
        ...
        top_n: Số từ khóa cần trả về
        language: Ngôn ngữ
    
    Returns:
        List các tuple (keyword, score)
    """
